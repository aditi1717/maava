  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (step2.profileImage instanceof File) {
        profileImageData = await handleUpload(step2.profileImage, "tuggo/restaurant/profile")
      ...
      for (const file of step2.menuImages.filter(f => f instanceof File)) {
        const uploaded = await handleUpload(file, "tuggo/restaurant/menu")
      ...
      if (step3.panImage instanceof File) {
        panImageData = await handleUpload(step3.panImage, "tuggo/restaurant/pan")
      ...
      if (step3.gstRegistered && step3.gstImage) {
        if (step3.gstImage instanceof File) {
          gstImageData = await handleUpload(step3.gstImage, "tuggo/restaurant/gst")
      ...
      if (step3.fssaiImage instanceof File) {
        fssaiImageData = await handleUpload(step3.fssaiImage, "tuggo/restaurant/fssai")

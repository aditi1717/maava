...
const buildZoneRestaurantMatch = (zoneDoc, zoneIdRaw) => {
    if (!zoneDoc || !zoneIdRaw || !mongoose.Types.ObjectId.isValid(zoneIdRaw)) return null;
...
export const listApprovedRestaurants = async (query = {}) => {
    ...
    if (!zoneFilterMatch && lat !== null && lng !== null) {
        const activeZones = await FoodZone.find({ isActive: true }).lean();
        const detectedZone = activeZones.find(z => doesZoneServePoint(lat, lng, z));
        if (detectedZone) {
            zoneFilterMatch = { zoneId: detectedZone._id };
        }
    }
